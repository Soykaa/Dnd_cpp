...
EndOfTheMove Board::isSomethingOnTheWay(Direction direction, int amountOfFields) {
	...
	if (direction == Direction::forward) {
		while(amountOfFields > 0 && Y < size_ - 1) {
			if (board_[X][Y].getType() != Type::emptyField) {
				if (board_[X][Y].getType() == Type::gift) {
					canWin = true;
					board_[X][Y].changeType(Type::emptyField);
					continue;
				}
				tmp.type = board_[X][Y].getType();
				tmp.x = X - 1;
				tmp.y = Y;
				return tmp;
			}
...
void Game::runGame() {
	...
	while (location_.getCharacterPosition_X() != 4 || location_.getCharacterPosition_Y() != 4 || !location_.getCanWin()) {
		...
		// Type t =  location_.makeTurn(direction, amountOfFields);
